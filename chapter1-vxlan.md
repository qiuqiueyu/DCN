...
（注：本地需在images文件夹存放对应图片，命名为cloud-architecture.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为spine-leaf.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为fusion-architecture.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为vxlan-overlay.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为vxlan-overhead.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为vxlan-packet.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为vxlan-central-gateway.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为subinterface-trunk.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为vtep-concept.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为vxlan-logic.png，GitHub可直接渲染）

五、亚罗实际物理组网

![亚罗实际组网](images/yalu-physical-network.png)

（注：本地需在images文件夹存放对应图片，命名为yalu-physical-network.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为bum-forward.png，GitHub可直接渲染）

七、VXLAN转发模式（对称 vs 非对称）

![转发模式对比](images/forward-mode.png)

（注：本地需在images文件夹存放对应图片，命名为forward-mode.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为switch-access.png，GitHub可直接渲染）
...
（注：本地需在images文件夹存放对应图片，命名为spine-multiactive.png，GitHub可直接渲染）
...
（待补充：VXLAN可手动配置，也可以通过EVPN自动建立，具体配置和原理后续补充）

十一、overlay&underlay

收集笔记

在构思笔记层中写下即时想法或大纲，亦或是会议记录或你对学习资料的注解

十二、堆叠

输出笔记

将构思笔记层中提炼出的需要执行的内容或最为核心的内容上移至行动笔记层，每次打开笔记文档时都能直指要点。
...
随着“即时”笔记越来越多，你可以将构思笔记层中多余或不重要的内容移动到封存笔记层，使构思笔记层保留精华内容，而封存笔记层可以作为补充信息查看
...
（注：本地需在images文件夹存放本章汇总图片，命名为charpter1-1.png，GitHub可直接渲染）
